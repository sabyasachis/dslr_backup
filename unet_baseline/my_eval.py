...
model = Unet(args, n_filters=64).cuda()
...
#model_file = 'second_attempt_triple_data_restarted_correctly_1024/models/gen_100.pth'
model_file = "gan_dynamic_fill/weights_saby_inpainting/checkpoint_60.pth"
print("Loading model from {}".format(model_file))

network=torch.load(model_file)['model']
...
#STATIC_PREPROCESS_DATA_PATH  =  "../training_data/small_map/dynamic_high_med_pedi/static_prepreprocess_dir"
...
if not os.path.exists("samples_vinny"):
    os.makedirs("samples_vinny/reconstructed")
    os.makedirs("samples_vinny/dynamic")
    os.makedirs("samples_vinny/static")
else:
    shutil.rmtree("samples_vinny")
    os.makedirs("samples_vinny/reconstructed")
    os.makedirs("samples_vinny/dynamic")
    os.makedirs("samples_vinny/static")
...
#     break
    ...
    for frame_num in range(recons_temp.shape[0]):
        ...
        plt.savefig('samples_vinny/reconstructed/'+str(i*args.batch_size+frame_num)+'.png') 
        # plt.show()
        plt.close()
    ...
    for frame_num in range(original_temp.shape[0]):
        ...
        plt.savefig('samples_vinny/dynamic/'+str(i*args.batch_size+frame_num)+'.png') 
        # plt.show()
        plt.close()
    ...
    for frame_num in range(staticc_temp.shape[0]):
        ...
        plt.savefig('samples_vinny/static/'+str(i*args.batch_size+frame_num)+'.png') 
        # plt.show()
        plt.close()

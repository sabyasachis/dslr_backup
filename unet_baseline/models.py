...
class DownBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, drop_rate=DEFAULT_DROP_RATE):
        super(DownBlock, self).__init__()
        self.down_double_conv = nn.Sequential(
            Down(),
            Doubleconv(in_channels, out_channels, drop_rate)
        )
    def forward(self, x):
...
class UpBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, drop_rate=DEFAULT_DROP_RATE):
        ...
        self.up   =         Up(    in_channels, out_channels, drop_rate)
        self.conv = Doubleconv( 2*out_channels, out_channels, drop_rate)
...
class Unet(nn.Module):
    def __init__(self, args, n_filters, return_last_layer=False):
        ...
        self.encoder_conv1 = Doubleconv(nc, ndf, drop_rate=DEFAULT_DROP_RATE)
        ...
        self.decoder_conv3 = Doubleconv( ndf*16*2, ndf*16, drop_rate=DEFAULT_DROP_RATE)

        self.decoder_up2 = UpBlock( ndf*16, ndf*8)
        self.decoder_up3 = UpBlock( ndf*8,  ndf*4)
        self.decoder_up4 = UpBlock( ndf*4,  ndf*2)
        self.decoder_up5 = UpBlock( ndf*2,  ndf, drop_rate=0.0)
...
class Unet_filtered(nn.Module):
    def __init__(self, args, n_filters):
        super(Unet_filtered, self).__init__()
        self.args = args
        nc = 3 if args.no_polar else 2
        ndf = n_filters

        self.unet = Unet(args, n_filters=n_filters, return_last_layer=True)
        self.out  = nn.Sequential(
                                  nn.Conv2d(nc*2 + ndf, ndf*2, 3, 1, 1, bias=False),
                                  nn.BatchNorm2d(num_features=ndf*2),
                                  nn.LeakyReLU(negative_slope=0.2, inplace=True),
                                  nn.Conv2d(ndf*2, 2, 3, 1, 1, bias=False),
                                  nn.Softmax(dim=1)
                                 )
...
class VAE_filtered(nn.Module):
    def __init__(self, args, n_filters):
        ...
        self.unet = VAE(args, n_filters=n_filters, return_last_layer=True)
        self.out  = nn.Sequential(
                                  nn.Conv2d(nc*2 + ndf, ndf*2, 3, 1, 1, bias=False),
                                  nn.BatchNorm2d(num_features=ndf*2),
                                  nn.LeakyReLU(negative_slope=0.2, inplace=True),

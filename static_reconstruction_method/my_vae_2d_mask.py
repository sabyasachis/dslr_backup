...
if(os.path.exists(OPTIM_PATH) and False):
...
class SemPairdata(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset):
        ...
        assert len(self.dataset_static_data) == len(self.dataset_dynamic_data)

    def __len__(self):
        return self.dataset_static_data.shape[0]

    def __getitem__(self, index):
        index1 = index
        return index, self.dataset_static_data[index1], self.dataset_dynamic_data[index1]
...
STATIC_PREPROCESS_DATA_PATH  =  "/home/saby/Projects/ati/ati_motors/adversarial_based/training_data/small_map/64beam/static_prepreprocess_dir"
DYNAMIC_PREPROCESS_DATA_PATH = "/home/saby/Projects/ati/ati_motors/adversarial_based/training_data/small_map/64beam/dynamic_prepreprocess_dir"
...
if PREPROCESS_FRESHLY:
    ...
    dataset_train_dict = {}
    for idx, file in tqdm(enumerate(npyList), total=len(npyList)):
        if idx != 0:
            continue
        dataset_train = {}
...
for epoch in range(epoch_start, 1000):
    ...
    recon_act_, recon_gwt_, recon_inwt_, mask_, total_ = [[] for _ in range(5)]
...
#         break
        ...
        for i, img_data in tqdm(enumerate(train_loader), total=len(train_loader)):
            # if i == 100:
            #    break
            static_img  = img_data[1].cuda()
            dynamic_img = img_data[2].cuda()
            ...
            loss_recon_act, loss_recon_gwt, loss_recon_inwt = recon_loss_fn(recon, static_img, mask)
            ...
            # assert False
            loss_recon = loss_recon_tot + (mask_factor * loss_mask)
            # if i == 0:
            #     print(loss_recon)
            #     assert False
...
#             print_and_log_scalar(writer, 'valid/recon', mn(recon_), writes)
    ...
    optim_state = {'epoch':epoch, 'mask_factor': mask_factor, 'optimizer': optim.state_dict()}
    torch.save(optim_state, new_optim_file)
    if os.path.exists(old_optim_file):
        os.remove(old_optim_file)

    if mask_factor < 10:
        mask_factor = mask_factor + 0.25
    gc.collect()

...
MODEL_PATH = 'second_attempt_filtered_32f_triple_data_restarted_again2_correctly_1024/models/gen_245.pth'
...
def slam_loss_fn(recon_curr, recon_next, gt_transform):
    ...
    for frame_num in trange(recon_curr.shape[0]):
...
#         assert False
        ...
        for i, img_data in tqdm(enumerate(train_loader), total=len(train_loader)):
            ...
            loss_recon = recon_loss_fn(recon_curr, static_img_curr) + recon_loss_fn(recon_next, static_img_next)
            ...
            assert False
            loss_recon = loss_recon + (mask_factor * loss_mask) + (slam_factor * loss_slam)
            # if i == 0:
            #     print(loss_recon)
            #     assert False

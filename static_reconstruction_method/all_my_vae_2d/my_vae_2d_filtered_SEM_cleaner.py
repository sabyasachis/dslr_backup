...
MODEL_PATH = './fifth_attempt_sem_ground_weighted_filtered_polar_old_unet_32f_1024_continued/models/gen_147.pth'
...
#assert False
#OPTIM_PATH = './fifth_attempt_sem_ground_weighted_filtered_polar_old_unet_32f_1024_continued/optims/gen_147.pth'
...
#    epoch_so_far = network_optim_dict['epoch']
#else:
#    assert False
gc.collect()
...
STATIC_PREPROCESS_DATA_PATH  =  "../training_data/small_map/SEM_8_24_48/static_prepreprocess_dir"
DYNAMIC_PREPROCESS_DATA_PATH = "../training_data/small_map/SEM_8_24_48/dynamic_prepreprocess_dir"
GT_PREPROCESS_DATA_PATH = "../training_data/small_map/SEM_8_24_48/gt_prepreprocess_dir"
...
def recon_loss_fn(recon, static):
    ...
    z_val = static[:,1].view((static.shape[0],1,static.shape[2],static.shape[3]))
    ...
    weighted_loss = ((recon - static) * loss_wt).abs().sum(-1).sum(-1).sum(-1)
    actual_loss = (recon - static).abs().sum(-1).sum(-1).sum(-1)
    return weighted_loss, actual_loss
...
def mask_loss_fn(dynamic, recon, mask, static):
    ...
    new_recon = (dynamic * (1-mask)) + (mask * recon)
...
for epoch in range(500):
    ...
    recon_wt_, recon_act_, mask_, slam_, total_ = [[] for _ in range(5)]
...
#         assert False
        ...
        for i, img_data in tqdm(enumerate(train_loader), total=len(train_loader)):
            ...
            loss_recon_wt_curr, loss_recon_act_curr = recon_loss_fn(recon_curr, static_img_curr)
            loss_recon_wt_next, loss_recon_act_next = recon_loss_fn(recon_next, static_img_next)
            loss_recon_wt = loss_recon_wt_curr + loss_recon_wt_next
            ...
            # assert False
            loss_recon = loss_recon_wt + (mask_factor * loss_mask) + (slam_factor * loss_slam)
            # if i == 0:
            #     print(loss_recon)
            #     assert False
            ...
            recon_wt_  += [loss_recon_wt.mean(dim=0).item()]
    ...
    print_and_log_scalar(writer, 'train/recon_wt_',  mn(recon_wt_),  writes)
...
#             print_and_log_scalar(writer, 'valid/recon', mn(recon_), writes)
            print_and_log_scalar(writer, 'valid/recon_wt_',  mn(recon_wt_),  writes)
    ...
    optim_state = {'epoch': epoch + 1, 'optimizer': optim.state_dict()}
    torch.save(optim_state, os.path.join(args.base_dir, 'optims/gen_{}.pth'.format(epoch)))

    if mask_factor < 0.75:
        mask_factor = mask_factor + 0.01
    if slam_factor < 100:
        slam_factor = slam_factor + 4
    gc.collect()

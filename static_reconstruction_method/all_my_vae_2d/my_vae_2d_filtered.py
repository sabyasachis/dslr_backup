...
MODEL_PATH = 'second_attempt_filtered_64f_triple_data_restarted_correctly_1024/models/gen_105.pth'
...
if PREPROCESS_FRESHLY:
    STATIC_TRAIN_FOLDER_PATH  = '/home/sabyasachi/Projects/ati/data/data/datasets/Carla/16beam-Data/small_map/pair_triple/static_out_npy/'
    DYNAMIC_TRAIN_FOLDER_PATH = '/home/sabyasachi/Projects/ati/data/data/datasets/Carla/16beam-Data/small_map/pair_triple/dynamic_out_npy/'
    ...
    static_dataset_train = np.load(os.path.join(STATIC_TRAIN_FOLDER_PATH, train_file))
    static_dataset_train = preprocess(static_dataset_train, LIDAR_RANGE).astype('float32')
    with open(os.path.join(STATIC_PREPROCESS_DATA_PATH, train_file), 'wb') as pfile:
    ...
    print("training dataset:")
    # for file in tqdm(npyList):
    dynamic_dataset_train = np.load(os.path.join(DYNAMIC_TRAIN_FOLDER_PATH, train_file))
    dynamic_dataset_train = preprocess(dynamic_dataset_train, LIDAR_RANGE).astype('float32')
    with open(os.path.join(DYNAMIC_PREPROCESS_DATA_PATH, train_file), 'wb') as pfile:
...
#         loss_fn = lambda a, b : (a - b).abs().sum(-1).sum(-1).sum(-1)
    ...
    for i, img_data in tqdm(enumerate(train_loader), total=len(train_loader)):
        ...
        loss_recon = recon_loss_fn(recon, static_img)
        loss_mask = mask_loss_fn(dynamic_img, recon, mask, static_img)
        loss_recon = loss_recon + (mask_factor * loss_mask)
        # if i == 0:
        #     print(loss_recon)
        #     assert False

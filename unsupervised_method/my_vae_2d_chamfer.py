...
if(os.path.exists('../prashVAE/small_map_unet_restarted/models/gen_94.pth')):
    network_state_dict = torch.load('../prashVAE/small_map_unet_restarted/models/gen_94.pth')
...
DYNAMIC_PREPROCESS_DATA_PATH = "../prashVAE/dynamic_prepreprocess_dir"
...
train_file = val_file   # Temporarily for testing purposes
...
if PREPROCESS_FRESHLY:
    ...
    #     static_dataset_train.dump(os.path.join(STATIC_PREPROCESS_DATA_PATH, file))
    ...
    # static_dataset_val.dump(os.path.join(STATIC_PREPROCESS_DATA_PATH, val_file))
    ...
    # static_dataset_train = np.load(os.path.join(STATIC_PREPROCESS_DATA_PATH, train_file), allow_pickle=True)
    
    print("Loading dynamic training dataset")
    dynamic_dataset_train = np.load(os.path.join(DYNAMIC_PREPROCESS_DATA_PATH, train_file), allow_pickle=True)
    ...
    dynamic_dataset_val = np.load(os.path.join(DYNAMIC_PREPROCESS_DATA_PATH, val_file), allow_pickle=True)
...
class ConsecutivePairdata(torch.utils.data.Dataset):
    """
    Dataset of numbers in [a,b] inclusive
    """

    def __init__(self, dataset):
        super(ConsecutivePairdata, self).__init__()
    ...
    def __getitem__(self, index):
        ...
        return index, self.dataset[index1],self.dataset[index2]
...
#         assert False
    ...
    for i, img_data in tqdm(enumerate(train_loader), total=len(train_loader)):
        ...
        loss_recon = loss_fn(recon_curr[:IMAGE_HEIGHT], recon_next[:IMAGE_HEIGHT])

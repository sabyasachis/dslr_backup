...
if(os.path.exists('first_attempt/models/gen_8.pth')):
    network_state_dict = torch.load('first_attempt/models/gen_8.pth')
...
for epoch in range(500):
    ...
    for key, dynamic_dataset_train in dynamic_dataset_train_dict.items():
        ...
        for i, img_data in tqdm(enumerate(train_loader), total=len(train_loader)):
            ...
            pose_list = [each for each in tqdm(process_pool.imap(parallel_icp, parallel_args),
            ...
            for frame_num in trange(dynamic_img_curr.shape[0]):
                ...
                recon_curr_frame_3d_flat_transformed  = torch.mm(pose_transform, recon_curr_frame_3d_flat)

                recon_curr_frame_3d_transformed = recon_curr_frame_3d_flat_transformed[:3].view(recon_next_frame_3d.shape)

                # Apply SLAM loss (like Chamfer Distance, etc) on the pair
                slam_step_loss = loss_fn(recon_curr_frame_3d_transformed, recon_next_frame_3d)

                # Append SLAM loss to the list
                slam_loss_list.append(slam_step_loss)
                gc.collect()

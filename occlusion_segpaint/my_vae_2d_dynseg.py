...
#print(summary(model, input_size=(2, 64, 1024)))
#gc.collect()
...
class add_noise_partial():
    def __init__(self):
        self.noise = [x/100 for x in range(5,40,5)]
    ...
    def __call__(self, sample):
        ...
        mask.view((2,-1))[0,indices] = 0
        mask.view((2,-1))[1,indices] = 1
        means = img.view((2, -1)).mean(-1)
        stds  = img.view((2, -1)).std(-1)
        noise_tensora = torch.zeros((numdyn//3)).normal_(0, noise)
        noise_tensorb = torch.zeros((numdyn//3)).normal_(0, noise)
        noise_tensorc = torch.zeros((2,numdyn//3)).normal_(0, noise)
        #means, stds = [np.expand_dims(np.expand_dims(np.expand_dims(x,0),-1),-1) for x in [means, stds]]
        means, stds = [x.unsqueeze(0).unsqueeze(-1).unsqueeze(-1) for x in [means, stds]]
         # normalize data
        norm = (img - means) / (stds + 1e-9)
        # add the noise
        norm.view((2,-1))[0][inda] = norm.view((2,-1))[0][inda] + noise_tensora
        norm.view((2,-1))[1][indb] = norm.view((2,-1))[1][indb] + noise_tensorb
        norm.view((2,-1))[:2,indc] = norm.view((2,-1))[:2,indc] + noise_tensorc
        # unnormalize
...
class Pairdata(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        img, mask = torch.as_tensor(self.dataset1[index]),torch.as_tensor(self.dataset2[index])
        if self.transform:
...
DYNAMIC_PREPROCESS_DATA_PATH = "/home/saby/Projects/abhinav/transfer/dynamic_2/preprocess"#"/home/saby/Projects/ati/data/data/datasets/Carla/64beam-Data/pair_transform_dynseg_all/segmentation_preprocess/static_preprocessed/"
PATH_FOR_MASK_SAVING_AND_LOADING = "/home/saby/Projects/abhinav/transfer/dynamic_2/preprocess_mask"#"/home/saby/Projects/ati/data/data/datasets/Carla/64beam-Data/pair_transform_dynseg_all/segmentation_preprocess/static_mask_npy/"


if PREPROCESS_FRESHLY:
    # Un preprocessed path
    # input to model
    DYNAMIC_TRAIN_FOLDER_PATH = "/home/saby/Projects/abhinav/transfer/dynamic_2"#"/home/saby/Projects/ati/data/data/datasets/Carla/64beam-Data/pair_transform_dynseg_all/static_out_npy/"
    # output of the model
    ...
    def making_mask(generated_run_npy):
        ...
        dynamic_only_array=dynamic_only_array.reshape(generated_run_npy.shape[0],1,64,1024)
        static_only_array=static_only_array.reshape(generated_run_npy.shape[0],1,64,1024)
...
#        dynamic_dataset_val = pickle.load(pkl_file)
    ...
    gc.collect()
    val_data = Pairdata(dynamic_dataset_val, mask_val)
...
for epoch in range(500):
    ...
    for idx, train_loader in enumerate(train_loader_list):
        ...
        for i, img_data in tqdm(enumerate(train_loader), total=len(train_loader)):
            # if i == 100:
            #    break
            #cv2.imshow("data",np.transpose(from_polar_np(img_data[1][0].numpy())[0],[1,2,0]))
    ...
    #         assert False
            ...
            loss_recon = loss_fn(xQ, static_img)

...
class VAE(nn.Module):
    def __init__(self, args):
        ...
        self.decoder_tanh_S             = nn.Sequential(nn.ReLU(True))      # Controversy on RELU vs Tanh here?

        self.decoder_conv2d_T = nn.Sequential(nn.Conv2d( nc * 2, ngf, 3, 1, 1, bias=False))
        self.decoder_relu_U   = nn.Sequential(nn.ReLU(True))
        self.decoder_conv2d_V = nn.Sequential(nn.Conv2d( ngf, nc, 3, 1, 1, bias=False))
...
#        xj = self.decoder_relu_j(xi)
        ...
        xS = self.decoder_tanh_S(xQ)
        
        x_cat5 = torch.cat([x, xS], dim=1)         # concat with input image x
        xT = self.decoder_conv2d_T(x_cat5)
        xU = self.decoder_relu_U(xT)
        xV = self.decoder_conv2d_V(xU)

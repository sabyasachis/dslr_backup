...
class VAE(nn.Module):
    def __init__(self, args):
        ...
        self.decoder_convtranspose2d_16 = nn.Sequential(nn.ConvTranspose2d(ngf * 8, ngf * 4, (4,4), stride=2, padding=(1,1), bias=False))
    ...
    def forward(self, x):                      # Input size : (2, 32, 1024)
        # Encoder network forward pass        
        xa = self.encoder_conv2d_a(x)          # Conv : (64, 32, 1024)
        xb = self.encoder_leakyrelu_b(xa)

        x1 = self.encoder_conv2d_1(xb)         # Conv w/ stride=2 : (64, 16, 512)
        x2 = self.encoder_leakyrelu_2(x1)
        xc = self.encoder_conv2d_c(x2)         # Conv : (64, 16, 512)
        xd = self.encoder_leakyrelu_d(xc)

        x3 = self.encoder_conv2d_3(xd)         # Conv w/ stride=2 : (128, 8, 256)
        x4 = self.encoder_batchnorm2d_4(x3)
        x5 = self.encoder_leakyrelu_5(x4)
        xe = self.encoder_conv2d_e(x5)         # Conv : (128, 8, 256)
        xf = self.encoder_leakyrelu_f(xe)

        x6 = self.encoder_conv2d_6(xf)         # Conv w/ stride=2 : (256, 4, 128)
        x7 = self.encoder_batchnorm2d_7(x6)
        x8 = self.encoder_leakyrelu_8(x7)
        xg = self.encoder_conv2d_g(x8)         # Conv : (256, 4, 128)
        xh = self.encoder_leakyrelu_h(xg)

        x9 = self.encoder_conv2d_9(xh)         # Conv w/ stride=2 : (512, 1, 64)
        x10 = self.encoder_batchnorm2d_10(x9)
        x11 = self.encoder_leakyrelu_11(x10)

        x12 = self.encoder_conv2d_12(x11)      # Conv for Bottleneck : (1024,1,1)
        z = x12
        ...
        x13 = self.decoder_convtranspose2d_13(x12)    # Upconv : (512, 1, 64)
...
#        xj = self.decoder_relu_j(xi)
        ...
        x16 = self.decoder_convtranspose2d_16(x15)    # Upconv : (256, 4, 128)
        x17 = self.decoder_batchnorm2d_17(x16)
        x18 = self.decoder_relu_18(x17)
        x_cat1 = torch.cat([x18, xh], dim=1)          # concat with xg/xh : (256, 4, 256)
        xk = self.decoder_conv2d_k(x_cat1)            # Conv : (256, 4, 128)
        xl = self.decoder_relu_l(xk)

        x19 = self.decoder_convtranspose2d_19(xl)     # Upconv : (128, 8, 256)
        x20 = self.decoder_batchnorm2d_20(x19)
        x21 = self.decoder_relu_21(x20)
        x_cat2 = torch.cat([x21, xf], dim=1)          # concat with xe/xf : (128, 8, 512)
        xm = self.decoder_conv2d_m(x_cat2)            # Conv : (256, 8, 256)
        xn = self.decoder_relu_n(xm)

        x22 = self.decoder_convtranspose2d_22(xn)     # Upconv : (64, 16, 512)
        x23 = self.decoder_batchnorm2d_23(x22)
        x24 = self.decoder_relu_24(x23)
        x_cat3 = torch.cat([x24, xd], dim=1)          # concat with xc/xd : (64, 16, 1024)
        xo = self.decoder_conv2d_o(x_cat3)            # Conv : (64, 16, 512)
        xp = self.decoder_relu_p(xo)

        x25 = self.decoder_convtranspose2d_25(xp)     # Upconv : (64, 32, 1024)
        x26 = self.decoder_relu_26(x25)
        x_cat4 = torch.cat([x26, xb], dim=1)          # concat with xa/xb : (64, 32, 2048)
        xq = self.decoder_conv2d_q(x_cat4)            # Conv : (2, 32, 1024)
        x27 = self.tanh_27(xq)
        recon = x27

    ...
    for epoch in range(500):
        ...
        for key, dynamic_dataset_train in dynamic_dataset_train_dict.items():
            ...
            for i, img_data in tqdm(enumerate(train_loader), total=len(train_loader)):
                ...
                mp.set_start_method('spawn', force=True)
                parallel_args = [(recon_curr[frame_num:frame_num+1, :, :IMAGE_HEIGHT, :], recon_next[frame_num:frame_num+1, :, :IMAGE_HEIGHT, :])\
                                    for frame_num in trange(dynamic_img_curr.shape[0])]

                print("Slamming ")
                process_pool = mp.Pool(dynamic_img_curr.shape[0])
                slam_pose_list = [each for each in tqdm(process_pool.imap(parallel_slam, parallel_args),
                ...
                for frame_num in trange(dynamic_img_curr.shape[0]):
                    ...
                    pose_transform = slam_pose_list[frame_num]
